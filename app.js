...
const STEP1_HINTS = [
  ...
  "힌트: 방금 고른 선택지는 아니에요. 남은 선택지를 비교해 보세요."
];
const ZONE_HINTS = {
  freedom:"힌트: 생각·양심·사생활·표현·참여처럼 스스로 선택하는 삶과 가까워요.",
  safe:"힌트: 차별, 생명, 신체의 안전, 법의 보호처럼 평등과 안전에 가까워요.",
  dignity:"힌트: 교육·노동·휴식·복지·주거·문화처럼 인간다운 생활 조건과 가까워요.",
  together:"힌트: 환경·재난·국제적 협력처럼 혼자 또는 한 나라만으로 해결하기 어려운 문제와 가까워요."
};
...
async function loadQuestions(){
  try{
    const ref=doc(db,"config","questions"), s=await getDoc(ref);
    const saved=s.exists()?s.data():null;
    if(saved && Array.isArray(saved.items) && saved.contentVersion===CONTENT_VERSION){
      questions=saved.items;
    }else{
      questions=structuredClone(DEFAULT_QUESTIONS);
      await setDoc(ref,{items:questions,contentVersion:CONTENT_VERSION,updatedAt:serverTimestamp()});
    }
...
$("#startBtn").onclick=async()=>{
  ...
  if(snap.exists()){
    ...
    if(d.progress?.completed){openModal(`<h2>이미 완료한 기록이 있어요.</h2><p>${cls}반 ${num}번 ${name} 학생은 이미 인권지도를 완성했습니다.</p><div class="row-actions"><button id="resumeDone" class="primary">완료 화면 보기</button><button id="restartDone" class="ghost">처음부터 다시 하기</button></div>`);$("#resumeDone").onclick=()=>{closeModal();currentStudent={id,cls,num,name};currentProgress=d.progress;routeCompletedStudent();};$("#restartDone").onclick=()=>confirmRestart(id,cls,num,name);return;}
...
async function createStudent(id,cls,num,name){
  ...
  currentStudent={id,cls,num,name};currentProgress={order,index:0,doneCount:0,completed:false,answers:{},zoneAnswers:{},prizeDraw:null,startedAt:nowISO(),completedAt:null};
...
function renderMap(){
  ...
  for(const z of Object.keys(ZONES)){const vals=Object.values(currentProgress.zoneAnswers||{}).filter(v=>v.zone===z&&v.done);$(`#count-${z}`).textContent=`${vals.length}/${ZONES[z].target}`;$(`#chips-${z}`).innerHTML=vals.map(v=>`<i>${v.short}</i>`).join("");}
}
...
function renderStep1(){
  ...
  $("#answerOptions").innerHTML=shuffle([currentQuestion.right,...currentQuestion.wrong]).map(x=>`<button>${x}</button>`).join(""); $$("#answerOptions button").forEach(b=>{b.disabled=false;b.onclick=()=>checkRight(b.textContent);});
}
async function checkRight(choice){
  answerAttempt++;
  const ok=choice===currentQuestion.right;
  const a=currentProgress.answers[currentQuestion.id]||{tries:[],firstCorrect:null,revealed:false};
  a.tries.push(choice);
  if(a.firstCorrect===null)a.firstCorrect=ok;
  currentProgress.answers[currentQuestion.id]=a;

  if(ok){
    $("#feedback").className="feedback good";
    $("#feedback").innerHTML=`<strong>${rand(settings.messages?.correct||DEFAULT_MESSAGES.correct)}</strong><br>${currentQuestion.explanation}`;
    $("#answerOptions").innerHTML="";
    $("#nextStepBtn").classList.remove("hidden");
    $("#nextStepBtn").textContent="인권지도에 분류하기";
    $("#nextStepBtn").onclick=renderStep2;
  }else{
    const clicked=$$("#answerOptions button").find(b=>b.textContent===choice);
    if(clicked){clicked.disabled=true;clicked.classList.add("wrong-disabled");}
    const remaining=$$("#answerOptions button").filter(b=>!b.disabled).length;
    let hint=STEP1_HINTS[Math.min(answerAttempt-1,STEP1_HINTS.length-1)];
    if(answerAttempt>=2) hint+=`<br><small>추가 힌트: ${currentQuestion.explanation}</small>`;
    $("#feedback").className="feedback warn";
    $("#feedback").innerHTML=`괜찮아! 다시 골라보자. 🌱<br>${hint}<br><small>남은 선택지 ${remaining}개</small>`;
  }
  await saveProgress();
}
function renderStep2(){
  ...
  $$(".zone").forEach(z=>{z.disabled=false;z.classList.remove("zone-wrong-disabled");z.onclick=()=>checkZone(z.dataset.zone);});
}
async function checkZone(zone){
  zoneAttempt++;
  if(zone===currentQuestion.zone){
    $("#feedback").className="feedback good";
    $("#feedback").textContent="좋아요! 인권지도에 추가되었습니다.";
    await completeQuestion(zone,false);
  }else{
    const wrongZone=document.querySelector(`.zone[data-zone="${zone}"]`);
    if(wrongZone){wrongZone.disabled=true;wrongZone.classList.add("zone-wrong-disabled");}
    $("#feedback").className="feedback warn";
    let text="이 영역은 아니에요. 다시 선택해보자! 🌱";
    if(zoneAttempt>=2) text+=`<br><small>${ZONE_HINTS[currentQuestion.zone]}</small>`;
    $("#feedback").innerHTML=text;
  }
}
function shortName(r){const m={"표현의 자유":"표현","종교·양심의 자유":"종교·양심","사생활을 보호받을 권리":"사생활","평화롭게 모이고 단체를 만들 자유":"모임·단체","정치에 참여할 권리":"정치 참여","차별받지 않을 권리":"차별금지","생명과 신체의 안전을 보호받을 권리":"생명·안전","강제로 붙잡히거나 갇히지 않을 권리":"자의적 구금 금지","고문이나 잔혹한 대우를 받지 않을 권리":"고문 금지","법 앞에서 평등하게 보호받을 권리":"법 앞의 평등","교육받을 권리":"교육","일하고 정당한 대우를 받을 권리":"노동","휴식할 권리":"휴식","사회보장과 필요한 도움을 받을 권리":"사회보장·도움","건강·주거를 포함해 인간다운 생활을 할 권리":"건강·주거","문화생활에 참여할 권리":"문화","깨끗하고 건강한 환경에서 살 권리":"환경","평화롭게 살아갈 권리":"평화","재난과 위기에서 도움을 받을 권리":"재난·위기 지원","국제적 연대와 협력을 통해 인권을 보장받을 권리":"국제적 연대·협력"};return m[r]||r;}
...
async function showResult(){
  if(currentProgress&&!currentProgress.completed){
    ...
    currentProgress.completedAt=nowISO();
    if(!("prizeDraw" in currentProgress)) currentProgress.prizeDraw=null;
    await saveProgress();
  }
  showScreen("#screen-celebrate");
}
function routeCompletedStudent(){
  if(currentProgress?.prizeDraw?.done){
    renderLuckyResult(currentProgress.prizeDraw.won);
    showScreen("#screen-lucky");
  }else{
    showScreen("#screen-celebrate");
  }
}
...
function renderLuckyResult(won){
  ...
  $("#luckyIntro").textContent="오늘의 행운 결과!";
  if(won){
    $("#drawResultImage").src="./assets/win_jelly.png";
    $("#drawResultImage").alt="고양이 발바닥 젤리";
    $("#drawResultTitle").textContent="🎉 당첨!";
    $("#drawResultText").innerHTML="<strong>간식 1개</strong>를 받을 수 있어요! 선생님께 화면을 보여주세요.";
  }else{
    $("#drawResultImage").src="./assets/lose_meme.png";
    $("#drawResultImage").alt="꽝";
    $("#drawResultTitle").textContent="꽝!";
    $("#drawResultText").textContent="오늘은 아쉽지만… 인권지도 완성 자체가 이미 대성공! 😎";
  }
}
$("#quitBtn").onclick=()=>showScreen("#screen-home");
$("#goLuckyBtn").onclick=()=>{
  if(currentProgress?.prizeDraw?.done){
    renderLuckyResult(currentProgress.prizeDraw.won);
  }else{
    $("#drawReady").classList.remove("hidden");
    $("#drawResult").classList.add("hidden");
    $("#luckyIntro").innerHTML="기회는 딱 한 번!<br>행운이 찾아올까요?";
  }
  showScreen("#screen-lucky");
};
...
$("#previewCloseBtn").onclick=()=>showScreen("#screen-done");
$("#doneHomeBtn").onclick=()=>showScreen("#screen-home");
...
$("#resetQuestionsBtn").onclick=async()=>{if(!confirm("모든 문제를 처음 기본값으로 되돌릴까요?"))return;questions=structuredClone(DEFAULT_QUESTIONS);await setDoc(doc(db,"config","questions"),{items:questions,contentVersion:CONTENT_VERSION,updatedAt:serverTimestamp()});renderQuestionEditors();toast("기본 문제로 되돌렸습니다.");};
